        ...
        if lines is None:
            ...
            for i1, l1 in enumerate(lines):
                for i2, l2 in enumerate(lines[:i1]):
                    ...
                    if 10 < angles_diff < 170:
                        x,y = isect_lines(l1, l2)
                        if borders[0]-0.2*(borders[1]-borders[0]) < x < borders[1]+0.2*(borders[1]-borders[0]) and borders[2]-0.2*(borders[3]-borders[2]) < y < borders[3]+0.2*(borders[3]-borders[2]):
                            corners_long.append([x,y])
                            #cv2.circle(rgb, (int(x), int(y)), 1, (255, 255, 255), -1)

            if len(corners_long) == 0:
                print "no corners"
                ...
                while len(corners_long)>0:
                    ...
                    votes = 1
                    i = 0
                    while i < len(corners_long):
                        x1, y1 = corners_long[i]
                        dist = math.sqrt((xm-x1)*(xm-x1)+(ym-y1)*(ym-y1))  # calculate distance of each point
                        if dist < 60:  # if distance is small enough, remove point and add to new list
                ...
                if len(corners) < 4:
                    print "could not find 4 points"

                else:
                    corner_points = np.array([[corners[0][0],corners[0][1]],[corners[1][0],corners[1][1]],[corners[2][0],corners[2][1]],[corners[3][0],corners[3][1]]], dtype="double")
                    ...
                    (center_point_2D_base, jacobian_1) = cv2.projectPoints(np.array([(.0, .0, 0)]), rotation_vector, translation_vector, camera_matrix, dist_coeffs)
                    (center_point_2D_back, jacobian_2) = cv2.projectPoints(np.array([(.0, .0, square_side)]), rotation_vector, translation_vector, camera_matrix, dist_coeffs)
                    (center_point_2D_frnt, jacobian_2) = cv2.projectPoints(np.array([(.0, .0, -square_side)]), rotation_vector, translation_vector, camera_matrix, dist_coeffs)
