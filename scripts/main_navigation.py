...
def callback_visual_gate_dynamic_changed(input_data):
    ...
    if current_state.openloop_data.period is None:
        if gate_detection_dynamic_counter < 5:
            ...
            if detection_dynamic_input_history.shape[1] > 8:
                ...
                if std_deviation < 0.3:
                    ...
                    current_state.openloop_data.period = np.mean(periods)
        ...
        angle_theory = t_diff / (2*math.pi*current_state.openloop_data.period) + detection_dynamic_input_history[1][-1]
        ...
        if diff < 20 * math.pi / 180:
            ...
            a_delta = t_delta / (2*math.pi*current_state.openloop_data.period)
            ...
            current_state.openloop_data.theta = current_angle
            rospy.loginfo("current_state.openloop_data.theta")
            rospy.loginfo(current_angle)
        else:
            rospy.loginfo("discard detected pointer")

    rospy.loginfo("visual hand period")
    rospy.loginfo(current_state.openloop_data.period)
...
def callback_visual_gate_detection_changed(data):
    ...
    if wp_average is None:
        ...
        if current_state.openloop_data.state == 63 or 64:
            global wp_average1
            global wp_average2
            global wp_input_history2
            # now, add to list only if gate position is close to one of the averages
            distance1 = np.linalg.norm(wp_current.pos - wp_average1.pos)
            distance2 = np.linalg.norm(wp_current.pos - wp_average2.pos)
            hdg_diff1 = abs(
                math.atan2(math.sin(wp_current.hdg - wp_average1.hdg), math.cos(wp_current.hdg - wp_average1.hdg)))
            hdg_diff2 = abs(
                math.atan2(math.sin(wp_current.hdg - wp_average2.hdg), math.cos(wp_current.hdg - wp_average2.hdg)))
            if distance1 < 0.4 and hdg_diff1 < 15 * math.pi / 180:
                rospy.loginfo("use detected gate in front")
                wp_input_history.append(wp_current)
                del wp_input_history[0]
                wp_average1 = cr.find_average(wp_input_history)
                wp_average = cr.find_average([wp_average1, wp_average2])
                wp_average.hdg = math.atan2(wp_average2[1] - wp_average1[1], wp_average2[0] - wp_average1[0])
                rospy.loginfo("wp_average")
                rospy.loginfo(wp_average)
            elif distance2 < 0.4 and hdg_diff2 < 15 * math.pi / 180:
                rospy.loginfo("use detected gate in back")
                wp_input_history2.append(wp_current)
                del wp_input_history2[0]
                wp_average2 = cr.find_average(wp_input_history2)
                wp_average = cr.find_average([wp_average1, wp_average2])
                wp_average.hdg = math.atan2(wp_average2[1] - wp_average1[1], wp_average2[0] - wp_average1[0])
                rospy.loginfo("wp_average")
                rospy.loginfo(wp_average)
...
def calculate_visual_wp():
    ...
    if state_auto == 72:
        ...
        rospy.loginfo("state 62, calculate hover position")
        ...
        rospy.loginfo("state 63, calculate hover position")
...
def navigate_jungle():
    ...
    rospy.loginfo("JUNGLE - openloop state")
    rospy.loginfo(current_state.openloop_data.state)

    # stabilize
    if current_state.openloop_data.state == 0:
        # start a timer
    ...
    # Go
        ...
        throttle_executer(0.5, 2.0)
        current_state.openloop_data.state = 4
        # at this point a break message will be returned
        rospy.loginfo("JUNGLE - second break command")

    # wait
    elif current_state.openloop_data.state == 3:
        # wait here and don't return a msg
        while current_state.openloop_data.state == 3:
            time.sleep(0.1)
        rospy.loginfo("JUNGLE - stacked break command")

    # brake
    elif current_state.openloop_data.state == 4:
        if state_bebop == 2:
            # advance own state to advance state machine
            rospy.loginfo("JUNGLE - completed")
            current_state.openloop_data.state = 5
        else:
            # send brake command
            rospy.loginfo("JUNGLE - real break command")
    return msg


def throttle_executer(amount, duration):
...
def navigate_dynamic():
    ...
    rospy.loginfo("DYN - openloop state")
    rospy.loginfo(current_state.openloop_data.state)
    ...
    # wait for gate rotation
        ...
        if current_state.openloop_data.period is None:
            ...
            angle_difference = abs(current_state.openloop_data.theta-current_state.openloop_data.theta_trigger())
            if angle_difference < abs(2*math.pi/(current_state.openloop_data.period*5)*.7):
                rospy.loginfo("DYN - pointer angle triggered, state 3")
                current_state.openloop_data.state = 3
                # during execution of full throttle other instances of the callback go into mode 3 and wait there
                throttle_executer(1.0, 1.4)
                current_state.openloop_data.state = 4
                # at this point a break message will be returned
...
class State:
    def __init__(self, own_state=None, next_state=None, condition_type=None, condition_thres=None,
        ...
        if condition_type == "open":
            self.openloop_data = cr.OpenloopData()
    ...
    def enter(self):
        ...
        if self.jungle_on and self.own_state == 63:
            ...
            global wp_average1
            wp_average1 = wp_average
            for idx, wp in enumerate(wp_input_history2):
                wp_input_history2[idx] = wp_move(wp, 0.7, wp_average.hdg)
            global wp_average2
            wp_average2 = cr.find_average(wp_input_history2)
            global wp_average
            wp_average = cr.find_average([wp_average1, wp_average2])
    ...
    def check(self, navigation_distance):
        ...
        elif self.condition_type == "open":
            if self.openloop_data.state == self.condition_thres:
                self.exit()
...
if __name__ == '__main__':
    ...
    states[73] = State(73, 80, "open",  5,                1, 1, d, d, None, [], [])
